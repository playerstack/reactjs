...
Player Stack is and will remain open source, for a higher rate of fixes and releases over time. Thanks to all community members for their continued support.
...
<script>
  ...
  renderPlayerStack(container, { url, playing: true });

  function pausePlayer() {
    renderPlayerStack(container, { url, playing: false });
  }
</script>
```
